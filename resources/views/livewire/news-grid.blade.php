<div>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
        <button wire:click="setCategory('')"
                class="px-3 py-1 rounded text-sm {{ $category == '' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200' }}">
            Todas
        </button>
        @foreach($categorias as $cat)
            <button wire:click="setCategory('{{ $cat }}')"
                    class="px-3 py-1 rounded text-sm {{ $category == $cat ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200' }}">
                {{ $cat }}
            </button>
        @endforeach
    </div>
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        @foreach($tags as $t)
            <button wire:click="setTag('{{ $t }}')"
                    class="text-sm {{ $tag == $t ? 'text-blue-600 font-semibold' : 'text-gray-600 dark:text-gray-300' }} hover:underline">
                #{{ $t }}
            </button>
        @endforeach
        @if($tag)
        ...
        @foreach($posts as $post)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow hover:shadow-xl transition overflow-hidden flex flex-col">
                ...
                <div class="p-4 flex flex-col flex-1">
                    <span class="text-xs bg-blue-500 text-white rounded px-2 py-1 self-start">
                        {{ $post['categoria'] }}
                    </span>
                    <h3 class="text-lg font-semibold mt-2 text-zinc-900 dark:text-white">
                        {{ $post['titulo'] }}
                    </h3>
